...
```
## 🏡 Homeworks: Control Structures and Functions

1. `for` Loop + Conditional: Gene Expression Classifier
...
Loop over the gene names and print:
`<Gene> has <Low/Moderate/High> expression`
...
2. `next`: Sample Screening Skips Bad Samples
...
Use a `for` loop with `next` to skip `bad` samples and print a message for others:
`Sample X is flagged for analysis`

3. `break`: Stop at Contaminated Sample
Use the same samples vector above.

**Task:**

Print `Analyzing sample X` for each sample.
But if a `contaminated` sample is found, stop processing immediately and print `Contamination detected! Halting...`

4. `while` Loop: Sample Prep Countdown
...
`Sample <i> is ready for analysis`
from 1 to 5.

5. `while` + Condition: Until Threshold
You’re measuring a protein level that starts at 2.
With each measurement, it increases randomly between 0.5 and 1.5 units.

**Task:**
Use a while loop to simulate the increase until the protein level exceeds 10.
Print each step.
```{r}
level <- 2
#your code goes here
...
6. Custom Function: Classify BMI
...
7. Function: Gene Risk Calculator
...
Make a data frame of 4 genes and expression values.
Add a new column category using a for loop that assigns `High`, `Moderate`, `Low` based on expression.

9. Function + Error Checking
Write a function `get_expression_level(gene, df)` that:
