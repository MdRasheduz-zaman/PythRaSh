...
```
# Prelude  
Well, why `git`and `GitHub` at the very beginning? I want you to be able to interact with me for all the lessons, take part in the discussion and make you competent with basic tasks (`push`and `pull`). You actually need 4-5 lines of code to push and pull. Just get used to it. You can push the codes from the class, or your own practice codes. You will feel better.  
...
**Imagine this situation:** You made a working version of whatever you are doing/making. But then you were trying to polish it more. And alas, your so-called polishing is messing you up! You don't know what is wrong! You closed the file many times and you can't just *undo* and *redo* things. What would you do? Here comes `git` in your rescue. It kept the snapshot of your previous working version(s) in GitHub (if pushed) and you can easily fall back and forth (`clone`/`download`/`pull` it in your machine again). Maybe, upload and downloads make a bit more sense, rather than `push` and `pull`/`clone`!
...
Now, we are adding the public key to GitHub.  
...
::: callout-warning
## Stop!
Did you copy-pasted it correctly?
:::
...
**Paste the key** you copied and save it. You are done.  

::: callout-warning
## Stop!
Did you copy-pasted it correctly and is the key visible if you check under `SSH and GPG keys` now? It shows only the second field of your key though. If not, it is not done correctly.
:::
...
::: callout-note
## Diversion
`echo "# Hello GitHub"` actually prints `# Hello GitHub` on the terminal/git bash (you can check by copy-pasting only this part). The `>` operator is sending `# Hello GitHub` to the file named `README.md`. We did not need to create this file from before. See the power of `echo`, `<` together.
:::
...
::: {.callout-note collapse="true"}
It shows the status of the files and folders. You see the untracked files here. Untracked files mean the ones those are different than your GitHub repo.  
:::
...
```
::: callout-note
## How are the things with `#` sign?
Those are comments to tell you about the commands task (or anything). One just need to put them after `#` sign.
:::
...
The `.` means **"everything"** inside the current folder/directory. Well, you could run it all the time instead of mentioning specific file name (`README.md`) earlier.  
  
::: callout-note

## Note  
You can add and push as many files as you want. Just create/modify them inside the correct folder/directory and instead of `git add README.md`, use `git add .`. The dot here means all the files/folders. Then you could follow the next steps as usual. So, you could do:
:::
...
Then we should try to see which files are inside this folder, right?
...
I assume you have many files and folders inside `Downloads`. But you should see `folder1`, `folder2`, `folder3` among others now. Don't worry. Just run it. We are using `-p` flag/parameter with `mkdir` command. It says `mkdir` to make the directories, but if the folder name(s) exist, don't make any changes there. So, your files and folders are safe. But did you notice that I used the folder name `Downloads` after `ls` command now? Well, where are we? Inside the root directory (`~`). Since we are at our root directory, if we run only `ls`, it will list everything inside the root (`~`) directory. Did we wanted that? So, we have to say the folder name and `Downloads` is just a step forward from `~`. That's why we used `ls Downloads` to list everything inside the `Downloads` folder. We could go inside `Downloads` folder first and then run `ls` only. We would achieve the same thing. `Downloads` is just one-step far from our root, do you catch this logic? If you wanted to list everything inside the `folder3` folder, you could run `ls Downloads/folder3` from the `~` directory. Do you see its logic? If not, think about this:  
...
- Isn't the generation of your father, aunt(s) and uncle(s) the same?   

In our toy example above, `folder1`, `folder2`, `folder3` are on the same level/generation (they are children of `Downloads`). Similarly, `Downloads`, `Documents` and `Desktop` are on the same level (children of `~`). Makes sense? 
...
`ls` is equivalent of seeing things inside a folder in Windows. Did you catch it?
:::
...
Well, one step/folder behind, right?
:::
...
3. Navigate to `Downloads` again. Stay there. Next task is coming.   
4. Go to your GitHub account and make a repository named `hw_repo1`. **Clone** it inside your `Downloads` folder (navigate to this folder correctly after cloning). Add something (files, folder, whatever) in this folder (`hw_repo1`) and push following the standard workflow. Make as many changes and pushes you like.  
  
5. Go to your GitHub account again and make a repository named `hw_repo2`. **Clone** it inside your `Downloads` folder (you know how to be inside `Downloads`, right?) again (go inside the `hw_repo2` folder correctly after cloning, you know how). Add something (files, folder, whatever) in this folder (`hw_repo1`) again and push following the standard workflow. Make as many changes and pushes you like.  
  
6.  You learned these bash commands today:
