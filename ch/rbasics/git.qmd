...
**So, the summary goes first.** Once you finish reading, fall back here to remind you the important things and steps of working with `git`and `GitHub` and terminal.  

- `git` and `GitHub` configuration is a bit tedious process (to make it secure for us though), but we are supposed to do it only for once. We don't need to do it again and again.
